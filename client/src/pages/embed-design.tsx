...
export default function EmbedDesign({ embeddedContext, testerActions }: EmbedDesignProps = {}) {
  ...
  useEffect(() => {
    console.log("[EmbedDesign] CP1 host", {
      ...
      rawUseIsMobile: rawIsMobile,
      usePhoneLayout: isMobile,
    });
  }, [isTopLevelHost, isMobile, rawIsMobile]);
  ...
  }, [useAopCustomizer, openAopPlacer, usesFlatOnTheFlyPreview]);
  ...
  const handleHoodieAopApply = useCallback(async (
    ...
    // Persist print-ready panels (body pillow → 150 DPI). Mockup API keeps
    // the 1800px rasters so Printers Mockup stays fast. Tester renders the
    // full files inside persistWork so Apply is not blocked.
    ...
    const persistPrintPanels = async () => {
      ...
      if (isAdminTester) await persistWork;
    };

    // Preview Studio: unlock Send a Test Order as soon as preview-size panels
    // are hosted. Skip full-res bakes and Printify lifestyle mockups.
    if (isAdminTester) {
      await persistPrintPanels();
      const frontCanvas = result.renderView("front");
      if (frontCanvas) {
        try {
          const frontHosted = await ensureHostedUrl(frontCanvas.toDataURL("image/jpeg", 0.85));
          setAopPatternUrl(frontHosted);
          setPrintifyMockupImages([{ url: frontHosted, label: "front" }]);
          setPrintifyMockups([frontHosted]);
          setMockupFailed(false);
          setMockupError(null);
          setMockupsStale(false);
        } catch (err: any) {
    ...
    if (isStorefront && savedJobIdRef.current && panelSaveShop) {
      emitTesterDesignStatus({
        jobId: savedJobIdRef.current,
        aopPanels: "saving",
      });
                ...
                onApply={async (appliedPatternUrl: string, options) => {
                  const nextPlacement: AopPlacementSettings | undefined = options.placementSettings || options.perPanelTransforms || options.panelRenderConfig
                    ? {
                        ...
                        hoodieSeamBleedPx: options.placementSettings?.hoodieSeamBleedPx ?? aopPlacementSettings?.hoodieSeamBleedPx,
                        ...
                        hoodiePatternSpecs: options.placementSettings?.hoodiePatternSpecs ?? aopPlacementSettings?.hoodiePatternSpecs,
                        applyAllover: options.placementSettings?.applyAllover ?? aopPlacementSettings?.applyAllover,
