...
function canvasToUploadDataUrl(
  canvas: HTMLCanvasElement,
  maxDim = MAX_PANEL_MOCKUP_PX,
  mockupFlattenColor = "#ffffff",
  preserveExactColor = false,
  bakePrintBackground = true,
): string {
  let w = canvas.width;
  let h = canvas.height;
  const isPrintAsset = maxDim > MAX_PANEL_MOCKUP_PX;
  // Print-res assets (used for the persistent Printify fulfillment product) are flattened
  // onto the background colour ONLY when the user explicitly picked one (bakePrintBackground).
  // Without a picked bgColor the print PNG keeps its real alpha — on coloured blanks the
  // garment must show through the chroma-keyed transparency, not a baked white box.
  // Mockup JPEGs always flatten (JPEG has no alpha channel).
  const encode = (source: HTMLCanvasElement) => {
    if (isPrintAsset && !bakePrintBackground) return source.toDataURL("image/png");
    const flattened = document.createElement("canvas");
    flattened.width = source.width;
    flattened.height = source.height;
    const fctx = flattened.getContext("2d");
    if (!fctx) return source.toDataURL(isPrintAsset ? "image/png" : "image/jpeg", isPrintAsset ? undefined : 0.92);
    fctx.fillStyle = mockupFlattenColor;
    fctx.fillRect(0, 0, flattened.width, flattened.height);
    fctx.drawImage(source, 0, 0);
    if (isPrintAsset || preserveExactColor) return flattened.toDataURL("image/png");
...
interface PatternCustomizerProps {
  ...
  aopTemplateId?: string | null;
  /** Selected garment colour. Used to flatten transparent mockup pixels so Printify does not show white print-area strips. */
...
export function PatternCustomizer({
  ...
  // Print-res PNGs only bake a background when the user explicitly picked one.
  // Otherwise the print file keeps its chroma-keyed transparency so coloured
  // blanks (blue hoodie, black tote, …) show through instead of a white box.
  const bakePrintBackground = Boolean(bgColor && bgColor !== "transparent");
  const getInactivePanelFillColor = useCallback(
    (position: string): string => {
      ...
      // No bgColor picked → fall back to garment colour for hoodie trim, so
      // trim doesn't render as white over a coloured hoodie.
      if (productKind === "hoodie" && isHoodieTrimPanel(position) && garmentColorHex) {
        return garmentColorHex;
      }
      // Never drop a panel outright — an omitted placeholder can leave print
      // areas without colour at fulfillment time. Default to white.
      return "#ffffff";
    },
    ...
    (value: number) => {
      ...
      if (productKind === "hoodie") {
        setHoodiePatternSpecs((prev) => {
          if (applyAllover) {
            const next: Partial<Record<HoodiePanelView, HoodiePatternSpec>> = { ...prev };
            for (const view of ["front", "back", "hood"] as const) {
              const base = prev[view] || prev.front || fallbackPatternSpec;
              next[view] = { ...base, tileInches: value };
            }
            return next;
          }
      ...
      const drawSlots = (slots: PanelSlot[], compositeW: number, compositeH: number) => {
        ...
        const resolvePatternTransform = (position: string): { t: PanelTransform; mirrorX: boolean; renderKey: string } => {
          const placeKey =
            productKind === "hoodie" && isHoodiePocketPanel(position)
              ? (getHoodiePocketTransformSourcePosition(position, panelPositions) || position)
              : position;
          const baseT = perPanelTransforms[placeKey] || { dxPx: 0, dyPx: 0, scalePct: 100 };
          const mirrorTarget = mirrorMode && isMirrorTarget(placeKey, slots);
          const sourcePos = mirrorTarget ? getMirrorSource(placeKey, slots) : null;
          const syncTarget =
            !sourcePos && syncSidesMode
              ? canonicalTransformPanelId(placeKey, true, panelPositions, productKind)
              : null;
          const syncT = syncTarget ? (perPanelTransforms[syncTarget] || baseT) : baseT;
          const isSyncedLeftPanel =
            !!syncTarget &&
            syncTarget !== placeKey &&
            placeKey.toLowerCase().includes("left");
          ...
          return {
            t: { ...rawT, scalePct: applyAllover ? alloverScalePct : rawT.scalePct },
            mirrorX: !!sourcePos,
        ...
        for (const slot of slots) {
          ...
          drawMaskedSlot(ctx, svgImg, sx, sy, sw, sh, (offCtx) => {
            ...
            offCtx.fillRect(0, 0, sw, sh);
            if (!renderArtwork) return;
            if (mirrorX) {
              const buf = document.createElement("canvas");
              buf.width = Math.max(1, Math.round(sw));
              buf.height = Math.max(1, Math.round(sh));
              ...
              offCtx.scale(-1, 1);
              offCtx.drawImage(buf, 0, 0, sw, sh);
              offCtx.restore();
            } else {
              drawTiledMotifInRect(
                offCtx,
  ...
  useEffect(() => {
    ...
    const onTouchStart = (e: TouchEvent) => {
      if ((mode !== "place" && mode !== "pattern") || e.touches.length !== 1) return;
  ...
  const updatePatternPanelScale = useCallback(
    (position: string, scalePct: number) => {
      if (mode === "pattern" && applyAllover) {
        setPerPanelTransforms((prev) => {
          const next: Record<string, PanelTransform> = { ...prev };
          for (const panel of panelPositions) {
            const key =
              productKind === "hoodie" && isHoodiePocketPanel(panel.position)
                ? (getHoodiePocketTransformSourcePosition(panel.position, panelPositions) || panel.position)
                : panel.position;
            next[key] = { ...(next[key] || { dxPx: 0, dyPx: 0, scalePct: 100 }), scalePct };
          }
          return next;
        });
        return;
      }
      updatePanelTransform(position, { scalePct });
    },
    [applyAllover, mode, panelPositions, productKind, updatePanelTransform],
  );

  const resetPatternPanelTransform = useCallback(
    (position: string) => {
      if (mode === "pattern" && applyAllover) {
        setPerPanelTransforms((prev) => {
          const next: Record<string, PanelTransform> = { ...prev };
      ...
      updatePanelTransform(position, { dxPx: 0, dyPx: 0, scalePct: 100 });
    },
    [applyAllover, mode, panelPositions, productKind, updatePanelTransform],
  );
  ...
  async function exportPanelImage(
    ...
    if (shouldFlipLeggingsLegSlot(productKind, pos.position)) {
      ...
      return canvasToUploadDataUrl(flipped, pixelCap, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground);
    }

    return canvasToUploadDataUrl(canvas, pixelCap, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground);
  }
    ...
    try {
      ...
      if (mode !== "place" || panelPositions.length === 0) {
        if (mode === "pattern" && panelPositions.length > 0) {
          ...
          const buildPatternAopPanelUrls = async (pixelCap: number): Promise<{ position: string; dataUrl: string }[]> => {
            ...
            const alloverScaleKey = canonicalTransformPanelId(activePanel, syncSidesMode, panelPositions, productKind);
            const alloverScalePct = alloverScaleKey ? (perPanelTransforms[alloverScaleKey]?.scalePct ?? 100) : 100;
            const resolveExportTransform = (position: string): { t: PanelTransform; mirrorX: boolean; renderKey: string } => {
              const placeKey =
                productKind === "hoodie" && isHoodiePocketPanel(position)
                  ? (getHoodiePocketTransformSourcePosition(position, panelPositions) || position)
                  : position;
              ...
              const groups =
                productKind === "hoodie"
                  ? buildCompositeLayout(getPanelGroup(position), panelPositions, svgImages).slots
                  : buildLinearPanelsLayout(panelPositions, 0).slots;
              const mirrorTarget = mirrorMode && isMirrorTarget(placeKey, groups);
              const sourcePos = mirrorTarget ? getMirrorSource(placeKey, groups) : null;
              const syncTarget =
                !sourcePos && syncSidesMode
                  ? canonicalTransformPanelId(placeKey, true, panelPositions, productKind)
                  : null;
            ...
            if (productKind === "leggings" && (syncSidesMode || mirrorMode)) {
              ...
              if (rightDef && leftDef) {
                ...
                if (syncSidesMode) {
                  ...
                  urls.push({ position: rightPos, dataUrl: canvasToUploadDataUrl(flipCanvas(cropR, outWR, outHR), pixelCap, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground) });
                  ...
                  urls.push({ position: leftPos, dataUrl: canvasToUploadDataUrl(flipCanvas(cropL, outWL, outHL), pixelCap, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground) });
                } else {
                  const rightExport = renderPanel(outWR, outHR, true);
                  urls.push({ position: rightPos, dataUrl: canvasToUploadDataUrl(rightExport, pixelCap, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground) });
                  urls.push({ position: leftPos,  dataUrl: canvasToUploadDataUrl(flipCanvas(rightExport, outWL, outHL), pixelCap, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground) });
                }
            ...
            for (const p of panelPositions) {
              ...
              if (mirrorX) {
                ...
                mirrored.width = outW;
                mirrored.height = outH;
                const mx = mirrored.getContext("2d")!;
                mx.translate(outW, 0);
                mx.scale(-1, 1);
                mx.drawImage(canvas, 0, 0);
                outForUpload = mirrored;
              }
              ...
              urls.push({ position: p.position, dataUrl: canvasToUploadDataUrl(outForUpload, pixelCap, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground) });
            }
            return urls;
          };
        ...
        const tiledDataUrl = canvasToUploadDataUrl(canvas, 4096, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground);
      ...
      const buildPlaceModePanelUrls = async (pixelCap: number): Promise<{ position: string; dataUrl: string }[]> => {
        ...
        for (const [leftPos, rightPos] of seamPairs) {
          ...
          panelUrls.push({ position: rightPos, dataUrl: canvasToUploadDataUrl(cropRight, pixelCap, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground) });
          ...
          panelUrls.push({ position: leftPos, dataUrl: canvasToUploadDataUrl(cropLeft, pixelCap, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground) });
      ...
      if (productKind === "leggings" && seamBleedPx > 0) {
        ...
        if (rightDef && leftDef) {
          ...
          panelUrls.push({ position: rightPos, dataUrl: canvasToUploadDataUrl(flipR, pixelCap, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground) });
          ...
          if (mirrorMode) {
            ...
            panelUrls.push({ position: leftPos, dataUrl: canvasToUploadDataUrl(mirrorCanvas, pixelCap, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground) });
      ...
      // For leggings mirror mode: left leg mirrors the right leg's artwork
        ...
        if (doMirror) {
          ...
          if (rightPanel) {
            ...
            panelUrls.push({ position: p.position, dataUrl: canvasToUploadDataUrl(mirrorCanvas, pixelCap, mockupFlattenColor, preserveExactPanelColor, bakePrintBackground) });
            continue;
          }
        }
  ...
  const activePanelT = transformEditPanelId
    ...
    : null;
  const patternScalePct =
    mode === "pattern" && applyAllover
      ? activePanelT?.scalePct ?? 100
      : activePanelT?.scalePct ?? 100;
        ...
        <div className="flex flex-col gap-4 min-w-0 overflow-x-hidden overflow-y-visible">
          ...
          {mode === "pattern" && (
            <>
              ...
              {productKind === "hoodie" &&
                ...
                buildCompositeLayout(activeView, panelPositions, svgImages).slots.length === 2 && (
                  <div className="grid grid-cols-2 gap-1.5">
                    <div className="flex items-center justify-between gap-2 rounded-md border-2 border-foreground/30 px-2 py-1.5 bg-background">
                      <Label htmlFor="pattern-hoodie-sync-sides" className="text-xs cursor-pointer">
                        Sync sides
                      </Label>
                      <Switch
                        id="pattern-hoodie-sync-sides"
                    ...
                    <div className="flex items-center justify-between gap-2 rounded-md border-2 border-foreground/30 px-2 py-1.5 bg-background">
                      <Label htmlFor="pattern-hoodie-mirror" className="text-xs cursor-pointer">
                        Mirror
                      </Label>
                      <Switch
                        id="pattern-hoodie-mirror"
                        checked={mirrorMode}
                        onCheckedChange={v => {
                          setMirrorMode(v);
              ...
              {productKind !== "hoodie" && (
                <div className="flex items-center justify-between gap-2 rounded-md border-2 border-foreground/30 px-2 py-1.5 bg-background">
                  <Label htmlFor="pattern-aop-mirror" className="text-xs cursor-pointer">
                    Mirror paired panel
                  </Label>
                  <Switch
                    id="pattern-aop-mirror"
                    checked={mirrorMode}
                    onCheckedChange={v => {
                      setMirrorMode(v);
                      if (v) setSyncSidesMode(false);
              ...
              {productKind === "leggings" &&
                panelPositions.some(p => isLeggingsLegSlot(p.position)) && (
                  <div className="flex items-center justify-between gap-2 rounded-md border-2 border-foreground/30 px-2 py-1.5 bg-background">
                    <Label htmlFor="pattern-aop-sync-sides" className="text-xs cursor-pointer">
                      Sync sides
                    </Label>
                    <Switch
                      id="pattern-aop-sync-sides"
                      checked={syncSidesMode}
                      onCheckedChange={v => {
                        setSyncSidesMode(v);
                        if (v) setMirrorMode(false);
