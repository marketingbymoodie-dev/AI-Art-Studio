/**
 * Harvest a per-size Wall Decal (759) flat print-shape mask via harvestFlatCalibration
 * and merge it into the existing canonical manifest (size-only geometryByBlank key).
 *
 *   npx tsx scripts/harvest-decal-size-mask.ts --size=12x18
 *
...
const BLUEPRINT_ID = CATALOG_SIZE_BLANK_BLUEPRINTS.wallDecals;
...
async function main() {
  const rawSize = argValue("size") || "12x18";
  const sizeKey = extractDimensionalKey(rawSize) || rawSize;
  if (!SIZE_KEYS.includes(sizeKey)) {
    throw new Error(`Unknown decal size ${rawSize}. Expected one of: ${SIZE_KEYS.join(", ")}`);
  }
  ...
  console.log(
    `[decal-mask] harvesting ${sizeKey} via harvestFlatCalibration (pt ${productTypeId}, ${storageKey})`,
  );
  ...
  if (!harvested || !maskUrl) {
    console.error(
      `[decal-mask] no geometryByBlank[${sizeKey}] mask. status=${result.status} error=${result.error || ""} keys=${Object.keys(result.manifest?.geometryByBlank || {}).join(",")}`,
    );
    process.exit(1);
  }
  ...
  console.log(`[decal-mask] merged ${sizeKey} mask into ${canonicalManifestPath(BLUEPRINT_ID, version)}`);
  console.log(`[decal-mask] maskUrl=${maskUrl}`);
  ...
  for (const pt of pts) {
    ...
    synced += 1;
    console.log(`[decal-mask] synced pt ${pt.id} (${pt.name})`);
  }
  console.log(`[decal-mask] done. ${sizeKey} mask ready on ${synced} product type(s).`);
  process.exit(0);
}

main().catch((err) => {
  console.error("[decal-mask]", err?.message || err);
  process.exit(1);
});
