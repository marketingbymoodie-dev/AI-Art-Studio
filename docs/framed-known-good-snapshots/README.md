...
| Adjustable Tote (AOP, folded) | 1300 | 2026-08-20 (staging) | `808f6a5` | [adjustable-tote-bp1300.md](./adjustable-tote-bp1300.md) |

VFP / HFP / tapestry share pin `23bfaab`. Shoulder Tote **836** is pinned at `c09b062`. Adjustable Tote **1300** is a different product (folded 2650×5250 fulfillment) pinned at staging `808f6a5`. Crew tee is pinned at `eafd244` (85% default scale, Print Side sync, dashed-guide AR). Fashion tee (bp 26) + baseball tee (bp 79) share `7d163e6`. Heavy Blend crewneck (bp 49) + hooded sweatshirt (bp 77) share `d677610` — **opaque-touch** trim warning baseline. Slim Phone Cases (bp **421**) pinned at `f253677` — mask-only preview bg, full-canvas bake / Printers Mockup. Prefer **surgical** reverts if only one product regresses.
