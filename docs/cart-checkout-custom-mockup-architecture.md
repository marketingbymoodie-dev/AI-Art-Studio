...
3. The backend uses the **Admin API** (OAuth `accessToken` for that shop) to:
   ...
   - **Reuse model:** one shadow per **generation job + catalog variant + print-config snapshot** (`published_products.designId` = `jobId::variantId::cfgHash`). Re-adding the **same print snapshot** in the same size/colour **increments** the existing cart line. A different background, print side, placement, or AOP group config mints its own shadow at **Add-to-Cart** (not on edit). A different size/colour also mints its own shadow (correct title + price). A new generation (new job) mints a new shadow. Do **not** key shadows by mockup URL — URL churn was creating a new product every preview refresh. Lookup of a cfg-keyed id must **not** fall back to `job::variant` or bare `job`. Legacy `job` / `job::urlhash` / `job::variant` rows are still found via lookup fallback **only when the incoming id has no print-config suffix**. Editing a saved design mutates the live job in place (no fork, no new gallery card, no new shadow). The cart line is frozen at ATC via `_flat_pl` / `_tote_pl` / `_aop_pl`.
   - **Multi-currency (do not "fix" this again):** Shopify Ajax cart prices (`/cart.js`, `/cart/add.js`) are in the **buyer's presentment currency** (e.g. AUD), while Admin API prices and the customizer's displayed retail are the **shop currency** (e.g. USD). $18.95 USD ≈ $27.00 AUD is **correct market conversion, not a stale price**. Never compare an iframe/Admin price to an Ajax cart line price to detect staleness — a 2026-08 "remint on mismatch" attempt did exactly that and minted a new shadow product on every AUD add (10s+ ATC, unprocessed product images in cart, base-variant fallbacks).
   - **Price on reuse:** write the customer-facing retail the customizer showed (front or both-tier). Live Admin front is the fallback when no price is sent. Stale shadows (e.g. $27 after the page shows $18.95) are rewritten on the next resolve. ATC always resolves — do not skip for PreShadow. Shadow **titles** must strip print-provider suffixes (`Printify Choice`, etc.).
...
| `extensions/theme-extension/assets/appai-art-embed.js` | ATC: prefer iframe’s already-resolved shadow when `variantId !== baseVariantId`; **re-resolve from base only** if cart add fails (expired / unpublished). Same **print snapshot** re-add increments the existing line (`_shadow_design_id` exact match). Different background/sides/placement is a new line. No merge on `_appai_job_id` alone. No cross-currency price comparisons (see multi-currency note above). Always requires HTTPS `_mockup_url` for shadow image. |
| `extensions/theme-extension/assets/appai-cart-guard.js` | Injects **`_mockup_url`**; keeps buyer-visible `mockup_url` / `image` props out. |
| `extensions/theme-extension/assets/appai-cart-images.js` | Replaces cart thumbs from `_mockup_url`. **All cart image links demoted** (`<a>` → `<span>`). Customizer/AppAI lines also demote **title** / `/products/` links so shoppers cannot open native/shadow PDPs. |
...
**Cart/checkout image invariant:** the line must add a **shadow variant** whose product image is the current design mockup (HTTPS). Line props alone do not drive native checkout thumbnails. AppAI cart lines must **not** navigate to the native/shadow PDP from cart image or title.
...
5. Flat ATC: after Apply, wait for PreShadow (console `[PreShadow] Shadow variant ready`) → ATC should skip a second Admin create; cart/checkout still show **this** design’s mockup.
6. Cart `/cart` + drawer: click image and title on an AppAI line — **no** navigation to `/products/…` (console `[AppAI Cart Image] installed 2.4`).
