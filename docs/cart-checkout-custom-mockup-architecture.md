...
3. The backend uses the **Admin API** (OAuth `accessToken` for that shop) to:
   ...
   - **Reuse model:** one shadow per **generation job + catalog variant** (`published_products.designId` = `jobId::variantId`). Re-adding the same design in the same size/colour **increments** the existing cart line. A different size/colour mints its own shadow (correct title + price). A new generation (new job) mints a new shadow. Do **not** key shadows by mockup URL — URL churn was creating a new product every preview refresh. Legacy `job` / `job::urlhash` rows are still found via lookup fallback **only when `baseVariantId` matches**.
...
| `extensions/theme-extension/assets/appai-art-embed.js` | ATC: prefer iframe’s already-resolved shadow when `variantId !== baseVariantId`; **re-resolve from base only** if cart add fails (expired / unpublished). Same-design re-add increments the existing line. No cross-currency price comparisons (see multi-currency note above). Always requires HTTPS `_mockup_url` for shadow image. |
