...
3. The backend uses the **Admin API** (OAuth `accessToken` for that shop) to:
   ...
   - **Reuse model:** one shadow per **generation job + catalog variant** (`published_products.designId` = `jobId::variantId`). Re-adding the same design in the same size/colour increments the existing cart line **when that line’s price still matches the customizer**. If Shopify locked an older price (e.g. $27 after the page shows $18.95), the theme **removes that line and adds fresh** so the live shadow price applies. A different size/colour mints its own shadow (correct title + price). A new generation (new job) mints a new shadow. Do **not** key shadows by mockup URL — URL churn was creating a new product every preview refresh. Legacy `job` / `job::urlhash` rows are still found via lookup fallback **only when `baseVariantId` matches**.
