...
3. The backend uses the **Admin API** (OAuth `accessToken` for that shop) to:
   ...
   - **Price on reuse:** re-read the live base **front** price, then apply a both-tier override only when Print Side is Both. Stale shadows (e.g. $27 after Admin is $18.95) are rewritten on the next resolve/PreShadow. ATC always resolves — do not skip for PreShadow.
