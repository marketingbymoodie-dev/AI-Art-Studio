...
| `extensions/theme-extension/assets/appai-cart-images.js` | Replaces cart thumbs from `_mockup_url`; **blocks** `/products/` navigation on AppAI/shadow lines (image + title). Detects via line props, `appai-shadow` product tag, and recent mockup seed; Horizon `cart-*-component` aware; strips `href` + `pointer-events:none` so cursor is not “active”. |
