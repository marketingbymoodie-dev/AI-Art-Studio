...
3. The backend uses the **Admin API** (OAuth `accessToken` for that shop) to:
   ...
   - **Reuse model:** one shadow per **generation job + catalog variant** (`published_products.designId` = `jobId::variantId`). Re-adding the same design **never increments** the existing cart line — Shopify locks line price at first add, so the theme **removes that line and adds fresh**. If `/cart/add.js` still returns the stale price for the same variant ID, the theme calls resolve with `forceNew` and mints a **new** shadow SKU (old DB row is updated to the new product IDs). A different size/colour mints its own shadow (correct title + price). A new generation (new job) mints a new shadow. Do **not** key shadows by mockup URL — URL churn was creating a new product every preview refresh. Legacy `job` / `job::urlhash` rows are still found via lookup fallback **only when `baseVariantId` matches**.
...
| `extensions/theme-extension/assets/appai-art-embed.js` | ATC: prefer iframe’s already-resolved shadow when `variantId !== baseVariantId`; **re-resolve from base only** if cart add fails (expired / unpublished). Never increment an existing AppAI line — remove and re-add. If the fresh add is still the old price, `forceNew` mints a new shadow SKU. Always requires HTTPS `_mockup_url` for shadow image. |
